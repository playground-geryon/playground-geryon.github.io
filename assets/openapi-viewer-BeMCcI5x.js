import{r as j,j as e}from"./vendor-CRMzTT1s.js";import{P as be,B as p,S as y,a as se,bo as ve,bp as ye,aQ as je,Y as S,bq as me,ae as we,ad as $e,a1 as Se,br as ke,T as d,y as Te,z as xe,g as de,bs as Ee,a4 as ze,a5 as Ce,a6 as Pe,a7 as Le,bt as qe,bu as Oe,f as Re,bv as Ae}from"./mui-BTFQHN8y.js";import"./excel-CUMakpxJ.js";import"./tree-sitter-aGyQ8e2w.js";const Q="#/components/schemas/",ae=(r,h)=>{if(!h||!h.$ref||typeof h.$ref!="string"||!h.$ref.startsWith(Q))return h;const f=h.$ref.slice(Q.length),m=r?.components?.schemas?.[f];return m?{...m,...h,$ref:void 0}:h},O=(r,h=0)=>{if(!r||h>4)return"...";if(r.$ref&&typeof r.$ref=="string")return`{${r.$ref.split("/").pop()??r.$ref}}`;const f=r?.type;return f==="array"||r?.items?`[${O(r?.items??{},h+1)}]`:f==="object"||r?.properties?`{ ${(r?.properties?Object.entries(r.properties):[]).map(([w,k])=>`${w}: ${O(k,h+1)}`).join(", ")} }`:r?.enum?.length?r.enum.map(m=>String(m)).join(" | "):f??"object"},ee=(r,h,f=0)=>{if(!r||f>3)return;const m=ae(h,r);if(m?.example!==void 0)return m.example;if(m?.default!==void 0)return m.default;const z=m?.type;if(m?.enum?.length)return m.enum[0];if(z==="object"||m?.properties){const w={},k=m.properties??{};return Object.entries(k).forEach(([R,B])=>{w[R]=ee(B,h,f+1)??null}),w}if(z==="array"||m?.items)return[ee(m.items??{},h,f+1)??null];switch(z){case"integer":case"number":return 1;case"boolean":return!0;case"string":default:return"string"}},We=async r=>{const h=await r.text();try{return JSON.parse(h)}catch{throw new Error("JSON 파일을 파싱할 수 없습니다.")}},c=r=>r==null?"":String(r).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;"),le=r=>{if(r==null)return"";if(typeof r=="string")return r;try{return JSON.stringify(r,null,2)}catch{return String(r)}},Ie=()=>{const[r,h]=j.useState(null),[f,m]=j.useState(null),[z,w]=j.useState(!1),[k,R]=j.useState(null),B=async E=>{w(!0),m(null);try{const $=await We(E);if(!$.openapi&&!$.swagger)throw new Error("openapi 또는 swagger 문서가 아닙니다.");h($),R(E.name)}catch($){m($?.message??"api-docs.json을 읽는 중 오류가 발생했습니다."),h(null),R(null)}finally{w(!1)}},oe=()=>{h(null),m(null),R(null)},A=j.useMemo(()=>({title:r?.info?.title,version:r?.info?.version,description:r?.info?.description}),[r]),Z=j.useMemo(()=>r?.servers??[],[r]),D=j.useMemo(()=>{if(!r?.paths)return[];const E=[];for(const[$,v]of Object.entries(r.paths)){const W=Array.isArray(v?.parameters)?v.parameters:[];for(const[L,C]of Object.entries(v)){const K=L.toUpperCase();if(!["GET","POST","PUT","PATCH","DELETE","HEAD","OPTIONS"].includes(K))continue;const N=Array.isArray(C?.parameters)?C.parameters:[],J={};[...W,...N].forEach(s=>{!s?.name||!s?.in||(J[`${s.in}:${s.name}`]=s)});const G=Object.values(J).map(s=>({name:s.name,in:s.in,required:s.required,description:s.description,schema:ae(r,s.schema),example:s.example??ee(s.schema,r)})),I=C?.requestBody,P=I?.content?Object.entries(I.content).map(([s,n])=>({mediaType:s,schemaName:typeof n?.schema?.$ref=="string"&&n.schema.$ref.startsWith(Q)?n.schema.$ref.slice(Q.length):void 0,example:n?.example??n?.examples?.[Object.keys(n?.examples??{})[0]]?.value??ee(n?.schema,r),exampleText:n?.example!==void 0?typeof n.example=="string"?n.example:JSON.stringify(n.example,null,2):O(n?.schema),schema:ae(r,n?.schema)})):[],U=Object.entries(C?.responses??{}).map(([s,n])=>({status:s,description:n?.description,contents:n?.content?Object.entries(n.content).map(([o,i])=>({mediaType:o,schemaName:typeof i?.schema?.$ref=="string"&&i.schema.$ref.startsWith(Q)?i.schema.$ref.slice(Q.length):void 0,example:i?.example??i?.examples?.[Object.keys(i?.examples??{})[0]]?.value??ee(i?.schema,r),exampleText:i?.example!==void 0?typeof i.example=="string"?i.example:JSON.stringify(i.example,null,2):O(i?.schema),schema:ae(r,i?.schema)})):[]})),te=C?.tags?.[0]||"default";E.push({path:$,method:K,summary:C?.summary,description:C?.description,tags:C?.tags??[],operationId:C?.operationId,primaryTag:te,parameters:G.length?G:void 0,requestBody:P.length?{description:I?.description,contents:P}:void 0,responses:U})}}return E},[r]),M=j.useMemo(()=>{if(r?.tags?.length)return r.tags.map($=>$.name||"").filter(Boolean);const E=new Set;for(const $ of D)$.primaryTag&&E.add($.primaryTag),$.tags?.forEach(v=>E.add(v));return Array.from(E)},[D,r?.tags]),F=j.useMemo(()=>{const E=r?.components?.schemas??{};return Object.entries(E).map(([$,v])=>({name:$,type:v.type,description:v.description,properties:v?.properties?Object.keys(v.properties):void 0,propertiesDetail:v?.properties?Object.entries(v.properties).map(([W,L])=>({name:W,typeText:O(L),description:L?.description,required:Array.isArray(v?.required)?v?.required.includes(W):!1})):void 0,shapeText:O(v),raw:v,example:v?.example,rawText:v?JSON.stringify(v,null,2):void 0,exampleText:v?.example!==void 0?typeof v?.example=="string"?v?.example:JSON.stringify(v?.example,null,2):void 0}))},[r]);return{spec:r,info:A,servers:Z,operations:D,tags:M,schemas:F,loading:z,error:f,fileName:k,handleFile:B,handleReset:oe,buildExportHtml:()=>{if(!r)return"";const E=s=>{if(!s)return"";try{return JSON.stringify(JSON.parse(s),null,2)}catch{const n=s.trim();let o="",i=0;const x=()=>"  ".repeat(Math.max(i,0));for(let b=0;b<n.length;b+=1){const T=n[b];T==="{"||T==="["?(o+=T+`
`,i+=1,o+=x()):T==="}"||T==="]"?(i-=1,o+=`
`+x()+T):T===","?o+=T+`
`+x():o+=T}return o}},$=(()=>{const s=[...M],n=new Map;D.forEach(x=>{const b=x.primaryTag||"default";n.has(b)||n.set(b,[]),n.get(b).push(x)});const o=Array.from(n.keys()).filter(x=>!s.includes(x));return[...s,...o].map(x=>({tag:x,ops:n.get(x)??[]})).filter(x=>x.ops.length>0)})(),W=`${k?.replace(/\.json$/i,"")||A.title||"openapi-doc"} - OpenAPI HTML`,L=s=>s?'<span class="pill-required">required</span>':'<span class="pill-optional">optional</span>',C=s=>{if(!s)return'<p class="muted">좌측 목록에서 경로를 선택하면 상세가 보입니다.</p>';const n=s.parameters??[];let o=`
    ...
    `;if(s.description&&(n+=`<div class="schema-desc">${c(s.description)}</div>`),s.shapeText){const o=E(s.shapeText);n+=`
