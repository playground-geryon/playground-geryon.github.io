...
***
___
`,Q=()=>{const[n,e]=b.useState(B),[i,d]=b.useState(50),[p,v]=b.useState(null),[x,L]=b.useState(!1),[k,$]=b.useState(null),M=b.useRef(null),j=b.useRef(!1),P=b.useRef(null),y=b.useMemo(()=>I(n),[n]),C=b.useMemo(()=>G(y),[y]),o=b.useMemo(()=>k?k.highlight(C,k.languages.markup,"markup"):C,[C,k]);b.useEffect(()=>{let a=!0;return(async()=>{const h=await W(()=>import("./prism-wcO-HLEn.js").then(R=>R.p),__vite__mapDeps([0,1,2]));await W(()=>import("./prism-wcO-HLEn.js").then(R=>R.c),__vite__mapDeps([0,1,2]));const f=h.default??h;a&&$(f)})(),()=>{a=!1}},[]),b.useEffect(()=>{const a=h=>{if(!j.current)return;const f=M.current?.getBoundingClientRect();if(!f)return;const R=h.clientX-f.left,z=Math.min(80,Math.max(20,R/f.width*100));d(z)},u=()=>{j.current=!1,L(!1),document.body.style.userSelect=""};return window.addEventListener("mousemove",a),window.addEventListener("mouseup",u),()=>{window.removeEventListener("mousemove",a),window.removeEventListener("mouseup",u)}},[]);const s=()=>{j.current=!0,L(!0),document.body.style.userSelect="none"},m=()=>`<!DOCTYPE html>
<html>
<head>
  <meta charset="UTF-8" />
  <style>
