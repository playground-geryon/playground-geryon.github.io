...
***
___
`,Q=()=>{const[n,e]=b.useState(W),[i,d]=b.useState(50),[p,v]=b.useState(null),[x,L]=b.useState(!1),[k,$]=b.useState(null),M=b.useRef(null),j=b.useRef(!1),C=b.useRef(null),y=b.useMemo(()=>I(n),[n]),P=b.useMemo(()=>G(y),[y]),o=b.useMemo(()=>k?k.highlight(P,k.languages.markup,"markup"):P,[P,k]);b.useEffect(()=>{let a=!0;return(async()=>{const h=await B(()=>import("./prism-wcO-HLEn.js").then(R=>R.p),__vite__mapDeps([0,1,2]));await B(()=>import("./prism-wcO-HLEn.js").then(R=>R.c),__vite__mapDeps([0,1,2]));const f=h.default??h;a&&$(f)})(),()=>{a=!1}},[]),b.useEffect(()=>{const a=h=>{if(!j.current)return;const f=M.current?.getBoundingClientRect();if(!f)return;const R=h.clientX-f.left,z=Math.min(80,Math.max(20,R/f.width*100));d(z)},u=()=>{j.current=!1,L(!1),document.body.style.userSelect=""};return window.addEventListener("mousemove",a),window.addEventListener("mouseup",u),()=>{window.removeEventListener("mousemove",a),window.removeEventListener("mouseup",u)}},[]);const s=()=>{j.current=!0,L(!0),document.body.style.userSelect="none"},m=()=>`<!DOCTYPE html>
<html>
<head>
  <meta charset="UTF-8" />
  <style>
...
</html>`;return{value:n,setValue:e,html:y,formattedHtml:P,highlightedHtml:o,splitPct:i,setSplitPct:d,containerRef:M,onSplitterDown:s,downloadHtml:()=>{const a=m(),u=new Blob([a],{type:"text/html;charset=utf-8"}),h=URL.createObjectURL(u),f=document.createElement("a");f.href=h,f.download="markdown-export.html",document.body.appendChild(f),f.click(),document.body.removeChild(f),URL.revokeObjectURL(h)},fileInputRef:C,resetSample:()=>{e(W),v(null)},openFilePicker:()=>C.current?.click(),handleFileChange:async a=>{const u=a.target.files?.[0];if(!u)return;const h=await u.text();e(h),v(u.name),a.target.value=""},currentFileName:p,isResizing:x}},re=()=>{const{value:n,setValue:e,html:i,splitPct:d,containerRef:p,onSplitterDown:v,downloadHtml:x,resetSample:L,fileInputRef:k,openFilePicker:$,handleFileChange:M,isResizing:j}=Q(),y=O().palette.mode==="dark"?"vs-dark":"vs";return t.jsxs(N,{sx:{p:1.25,display:"flex",flexDirection:"column",gap:1.25,flex:1,minHeight:785,height:"100%",width:1,maxWidth:"100%",overflowX:"hidden"},children:[t.jsx("input",{type:"file",accept:".md,.markdown,.txt,text/markdown",ref:k,style:{display:"none"},onChange:M}),t.jsx(X,{onUpload:$,onResetSample:L,onDownloadHtml:x}),t.jsxs(g,{ref:p,sx:{display:"flex",gap:0,overflow:"hidden",height:"calc(100vh - 235px)",width:"100%",maxWidth:"100%"},children:[t.jsx(g,{sx:{flex:`0 0 ${d}%`,minWidth:0,maxWidth:"100%",display:"flex",flexDirection:"column",pr:1},children:t.jsx(J,{value:n,onChange:e,editorTheme:y,disablePointerEvents:j})}),t.jsx(V,{onMouseDown:v}),t.jsx(g,{sx:{flex:1,minWidth:0,pl:1,display:"flex",flexDirection:"column",maxWidth:"100%"},children:t.jsx(K,{html:i,disablePointerEvents:j})})]})]})};export{re as default};
